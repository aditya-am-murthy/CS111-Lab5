...
#define EXT2_VALID_STATE 1
#define EXT2_CONTINUE_ERROR 1

#define ONE_SECOND_INTERVAL 1
...
void write_superblock(int fd) {
	...
	superblock.s_state             = EXT2_VALID_STATE; /* File system is clean */
	superblock.s_errors            = EXT2_CONTINUE_ERROR; /* Ignore the error (continue on) */
	superblock.s_minor_rev_level   = 0; /* Leave this as 0 */
	superblock.s_lastcheck = current_time; /* Last check time */
	superblock.s_checkinterval     = ONE_SECOND_INTERVAL; /* Force checks by making them every 1 second */
	superblock.s_creator_os        = EXT2_LINUX; /* Linux */
...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = USED_DIRS_COUNT;
...
void write_block_bitmap(int fd)
{
	...
	map_value[0] = 0xFF; //1s (setup)
	map_value[1] = 0xFF; //1s (setup)
	map_value[2] = 0x7F; //0 then 1s usable then (setup)
						 //blocks 23 to 1023 are usable
	map_value[127] = 0x80; //1 then 0s
...
void write_inode_bitmap(int fd)
{
	...
	map_value[0] = 0xFF; // 1s (setup)
	map_value[1] = 0x1F; // 000 then 1s (free + setup)
...
void write_inode_table(int fd) {
	...
	root_dir.i_mode = EXT2_S_IFDIR | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IXUSR 
	...
	//symbolic link pointing to hello woolrd called "hello"
	...
	memcpy((char*)hello.i_block, file_path, 12);
...
void write_root_dir_block(int fd)
{
	...
	//current and parent . abd ..

	struct ext2_dir_entry current_dir_entry = {0};
	struct ext2_dir_entry parent_dir_entry = {0};
	dir_entry_set(current_dir_entry,  EXT2_ROOT_INO,".");
	dir_entry_set(parent_dir_entry,EXT2_ROOT_INO,"..");
	dir_entry_write(current_dir_entry, fd);
	dir_entry_write(parent_dir_entry,fd);

	bytes_remaining -= current_dir_entry.rec_len;
	bytes_remaining -= parent_dir_entry.rec_len;

	//lost+found dir
	struct ext2_dir_entry lost_found_dir_entry = {0};
	dir_entry_set(lost_found_dir_entry, LOST_AND_FOUND_INO, "lost+found");
	dir_entry_write(lost_found_dir_entry, fd);

	bytes_remaining -= lost_found_dir_entry.rec_len;

	//hello world file
	struct ext2_dir_entry hello_world_dir_entry = {0};
	dir_entry_set(hello_world_dir_entry, HELLO_WORLD_INO, "hello-world");
	dir_entry_write(hello_world_dir_entry, fd);

	bytes_remaining -= hello_world_dir_entry.rec_len;

	//hello sym link
	struct ext2_dir_entry hello_dir_entry = {0};
	dir_entry_set(hello_dir_entry, HELLO_INO, "hello");
	dir_entry_write(hello_dir_entry, fd);

	bytes_remaining -= hello_dir_entry.rec_len;

	//pad
	struct ext2_dir_entry padding = {0};
	padding.rec_len = bytes_remaining;
	dir_entry_write(padding, fd);
}
